...
function download() {
  ...
  link.href =
    "https://github.com/cabalex/fnf-song-converter/wiki/Making-and-Importing-New-Songs#step-3--importing-your-song-map";
...
function loadFile(file) {
  ...
  reader.onload = (e) => {
    ...
    // Check if song is an object (fixes newer psych chart file format)
    if (typeof json.song === "object") json = json.song;
    ...
    for (i = 0; i < json.song.notes.length; i++) {
      ...
      beginsection_timing +=
        (((60 / bpm) * 4) / 16) * section.lengthInSteps * 1000;
      ...
      for (x = 0; x < section.sectionNotes.length; x++) {
        ...
        switch (noteHandlingOption) {
          ...
          case "expanded_truncate":
            addToNotesList(
              timing,
              `${Math.floor(note[1] / 8) * 8 + assignment[note[1] % 8]}_${
                note[2]
              }`
            );
            break;
          ...
          case "expanded":
            addToNotesList(
              timing,
              `${Math.floor(note[1] / 8) * 8 + assignment[note[1] % 8]}_${note
                .slice(2)
                .join("_")}`
            );
            break;
          case "expanded_unmodified":
            if (note[1] < 8) {
              addToNotesList(
                timing,
                `${assignment[note[1]]}_${note.slice(2).join("_")}`
              );
            } else {
    ...
    for (var i = 0; i < lookupArray.length; i++) {
      ...
      if (Object.keys(notesList).includes(lookupArray[i])) {
        for (var x = 0; x < notesList[lookupArray[i]].length; x++) {
          scratchList.push(
            `?${lookupArray[i]}_${notesList[lookupArray[i]][x]}`
          );
        }
      }
    }
